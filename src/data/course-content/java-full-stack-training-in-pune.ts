...
export const javaFullStackTrainingInPune: CourseRichContent = {
  ...
  curriculum: [
    ...
    },
    {
      ...
      highlight: true,
      description:
        "Building AI features in Java — a different skill from using an AI assistant to write code, and the one that is starting to separate candidates. It is placed last deliberately, because the roadmap this course follows is explicit that AI comes after solid backend foundations.\n\nYou start with LLM mechanics: prompts, system instructions, tokens, context windows, structured output, streaming, tool calling, and model selection. Then Spring AI — chat models, prompt templates, structured outputs, embeddings and vector stores.\n\nFrom there you build a full RAG pipeline: ingestion, chunking, embeddings, vector storage in PostgreSQL with pgvector, similarity search, context retrieval and grounded generation. Then agents — tool selection, multi-step workflows, memory, state, human approval, and exposing your own APIs and database as callable tools.\n\nIt closes on the two things production teams get wrong: AI security (prompt injection, data leakage, excessive permissions, tool authorisation, rate limiting, audit logging) and AI evaluation (groundedness, hallucination, retrieval quality, latency, token usage and cost).",
      topics: [
